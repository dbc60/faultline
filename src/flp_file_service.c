...
FL_FILE_OPEN_FN(flp_file_open) {
    ...
    case FL_FILE_WRITE:
        ...
        break;
    case FL_FILE_APPEND:
        // FILE_APPEND_DATA makes every WriteFile append atomically at end of
        // file; the OVERLAPPED offset is ignored on this handle.
...
// start offset; the call still completes synchronously. On an FL_FILE_APPEND handle,
// WriteFile ignores that offset and appends at end of file. A short count (including 0
// at EOF) is the EOF/error/fault signal. bytes is clamped to a single DWORD, so a caller
