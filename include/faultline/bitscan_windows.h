 ...
 * @brief 64-bit bit-scan primitives for the Windows/MSVC toolchain.
 * @version 0.2.0
 * @date 2026-08-09
 *
 * 32-bit MSVC provides only the 32-bit _BitScanForward and _BitScanReverse
...
static inline u08 fl_bit_scan_forward64(u32 *index, u64 mask) {
    ...
    if (_BitScanForward(&i, (unsigned long)(mask & 0xFFFFFFFFull))) {
        *index = (u32)i;
        found  = 1;
    } else if (_BitScanForward(&i, (unsigned long)(mask >> 32))) {
        *index = (u32)i + 32;
        found  = 1;
    }
#else
...
static inline u08 fl_bit_scan_reverse64(u32 *index, u64 mask) {
    ...
    if (_BitScanReverse(&i, (unsigned long)(mask >> 32))) {
        *index = (u32)i + 32;
        found  = 1;
    } else if (_BitScanReverse(&i, (unsigned long)(mask & 0xFFFFFFFFull))) {
        *index = (u32)i;
        found  = 1;
    }
#else
    if (_BitScanReverse64(&i, mask)) {
        *index = (u32)i;
        found  = 1;
    }
#endif
