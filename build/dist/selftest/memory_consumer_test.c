/**
 * @file memory_consumer_test.c
 * @author Douglas Cuthbertson
 * @brief Consumer-side test for the platform memory service, compiled against an
 *        *imported* memory package (not the repo's src/ tree).
 *
 * This is the recovered standalone platform memory-service test. It wires the
 * arena, fault injector, and fault memory context together by hand in a single
 * binary, injects the service, and verifies that malloc/calloc/realloc/free
 * route through the injected service and that an enabled fault injector makes
 * malloc fail. The in-driver suites cannot cover this single-binary assembly
 * because they always run as DLLs with the service injected by the driver.
 *
