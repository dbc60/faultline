...
:: Derive DIR_REPO from this script's location (build\cmd\fault_memory_service_dist.cmd)
SET DIR_CMD=%~dp0
...
SET DIR_DIST=%DIR_REPO%\dist\fault_memory_service
...
ECHO.
ECHO Done. Package written to dist\fault_memory_service\
ECHO.
ECHO   Consumer include paths:
ECHO     dist\fault_memory_service\include   (public headers)
ECHO     dist\fault_memory_service\src       (private headers; also covers src\fnv\)
ECHO   Compile these sources:
ECHO     dist\fault_memory_service\src\*.c
ECHO     dist\fault_memory_service\src\fnv\FNV64.c
