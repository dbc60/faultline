...
::
:: Build and optionally run the standalone platform memory service test executable.
::
:: Usage:
::   build\cmd\flp_memory_service_sa.cmd [build options...]
::
...
::   target\<vs>\<platform>\<build_type>\bin\flp_memory_service_sa_test.exe
:: and copied to test\ for easy execution.

SET PROJECT_NAME=Standalone Platform Memory Service
TITLE %PROJECT_NAME%
...
IF %build% EQU 1 (
    ...
    cl %CommonCompilerFlagsFinal% /experimental:c11atomics /wd4456 /DFL_EMBEDDED ^
        ...
        "%DIR_REPO%\standalone\flp_memory_service\src\flp_memory_service_sa_test.c" ^
        /Fo:"%DIR_OUT_OBJ%\\" ^
        /Fd:"%DIR_OUT_BIN%\flp_memory_service_sa_test.pdb" ^
        /Fe:"%DIR_OUT_BIN%\flp_memory_service_sa_test.exe" ^
    ...
    COPY /Y "%DIR_OUT_BIN%\flp_memory_service_sa_test.exe" "%DIR_REPO%\test\flp_memory_service_sa_test.exe" > NUL
    IF %verbose% EQU 1 ECHO Copied flp_memory_service_sa_test.exe to test\
)

IF %test% EQU 1 (
    IF %verbose% EQU 1 ECHO Run %PROJECT_NAME%
    "%DIR_REPO%\test\flp_memory_service_sa_test.exe"
