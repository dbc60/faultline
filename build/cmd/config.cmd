...
::
:: /experimental:c11atomics needs VS2022 17.5 or later. shell.cmd floors the
:: toolchain at VS2022, so every version this build accepts understands it. The
:: core allocator includes <stdatomic.h> unconditionally, so a toolchain without
:: C11 atomics cannot build this project at all.
::
:: Each branch sets CommonLinkerFlags for its architecture -- the flags below
:: are the complete set, so nothing may assign to it again afterwards. Per-project
:: libraries belong on the individual link lines, not here.
...
:: 32-bit build
:: cl %CommonCompilerFlags% "%DIR_REPO%\src\win32_all.c" /link /subsystem:windows,5.02 %CommonLinkerFlagsFinal%
