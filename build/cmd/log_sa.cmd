...
::
:: Build and optionally run the standalone log service test executable.
::
:: Usage:
::   build\cmd\log_sa.cmd [build options...]
::
...
::   target\<vs>\<platform>\<build_type>\bin\log_sa_test.exe
:: and copied to test\ for easy execution.

SET PROJECT_NAME=Standalone Log Service
TITLE %PROJECT_NAME%
...
IF %build% EQU 1 (
    ...
    cl %CommonCompilerFlagsFinal% /experimental:c11atomics /DFL_EMBEDDED ^
        ...
        /Fe:"%DIR_OUT_BIN%\log_sa_test.exe" ^
    ...
    COPY /Y "%DIR_OUT_BIN%\log_sa_test.exe" "%DIR_REPO%\test\log_sa_test.exe" > NUL
    IF %verbose% EQU 1 ECHO Copied log_sa_test.exe to test\
)

IF %test% EQU 1 (
    IF %verbose% EQU 1 ECHO Run %PROJECT_NAME%
    "%DIR_REPO%\test\log_sa_test.exe"
