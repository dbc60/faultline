...
- Shared API: `include/faultline/fl_file_service.h` - Service interface (`FLFileService` struct with open/read/write/close function pointers); `include/faultline/fl_file_types.h` - `FLFile` (opaque handle), `FLFileMode` (`FL_FILE_READ`/`FL_FILE_WRITE`/`FL_FILE_APPEND`)
...
- Contract shape: `open` takes a UTF-8 path and `FLFileMode`, returning `NULL` on failure. `read`/`write` are **positional** — an explicit byte offset, no implicit file pointer — so a handle is fully described by its arguments and safe to use from several threads at once; a short transferred count signals EOF, an error, or an injected fault. `FL_FILE_APPEND` maps to Win32 `FILE_APPEND_DATA`: every write ignores its offset and lands atomically at end of file, so several writers — including other processes appending to the same path — interleave whole records without the caller coordinating anything
- Deliberately has no notion of stdio: it models path-addressable file I/O with positional offsets, which doesn't map onto a stream. Consumers wanting console output alongside file output (e.g. the log service's `flp_log_set_output`) fall back to raw `<stdio.h>` for that path and route only the file-backed output through this service
- Consumers: the log service's path-based output (`flp_log_set_output_path`) and JUnit XML output (`output_junit.c`) both write through this service instead of raw `fopen_s`/`fprintf`, which makes their write paths exercisable by FaultLine's own fault injection
